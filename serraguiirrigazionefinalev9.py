...
while MODALITA not in ['sat-hadd', 'opt-hrmax']:
    MODALITA = input("Inserisci la modalità di enshp (Suboptimal: digitare 'sat-hadd' Optimal: digitare 'opt-hrmax'): ")
    if MODALITA not in ['sat-hadd', 'opt-hrmax']:
        print("Errore: modalità non valida. Riprova.")
...
def draw(state, action_text, screen, font, avanti_rect, indietro_rect, inizio_rect, fine_rect, idx, plan):
   ...
   # Scritta "Goal" con testo su più righe
    goal_font = pygame.font.Font(None, 20)
    goal_text = """
    Il goal è raggiunto 
    quando tutte le piante presenti
    raggiungono la domenica sera in salute, 
    ovvero con un livello alto di umidità.
    
    Lo stato inizale è il seguente:
    - se la pianta ha uno stato di umidità bassa,
      viene segnata in rosso
    - se la pianta ha uno stato di umidità alta,
      viene segnata in verde

    """
    goal_lines = goal_text.splitlines()  # Dividi il testo in righe

    # Posizione iniziale
    goal_x = width - 300  # Posizione orizzontale (a destra)
    goal_y = 100  # Posizione verticale iniziale

# Disegna ogni riga
    for i, line in enumerate(goal_lines):
        rendered_line = goal_font.render(line, True, BLACK)
        screen.blit(rendered_line, (goal_x, goal_y + i * 20))  # Incrementa la posizione verticale    # Giorno
        subtitle_font = pygame.font.Font(None, 32)
        giorno_text = subtitle_font.render(f"Giorno: {giorno.capitalize()}", True, BLACK)
    ...
    for i, plant in enumerate(state['piante']):
        um = state['umidita'].get(plant, 'bassa')
  ...
  # Calcola la posizione iniziale per centrare gli irrigatori
    ...
    for i, (irr, man) in enumerate(state['manutenzione'].items()):  # Itera solo sugli irrigatori effettivi
        ...
        usage_text = font.render(f"Utilizzo: {usage}", True, BLACK)
